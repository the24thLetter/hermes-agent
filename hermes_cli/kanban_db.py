...
def dispatch_once(
    ...
    *,
    ...
    def _resolve_ready_assignee(row: sqlite3.Row) -> Optional[str]:
        row_assignee = row["assignee"]
        if row_assignee:
            return str(row_assignee)
        # Honour kanban.default_assignee: when the dispatcher hits an
        # unassigned ready task and an operator-configured fallback exists,
        # persist the assignment and proceed. Dry-run reports the would-be
        # assignment without mutating the DB.
    ...
    def _profile_spawnable(row: sqlite3.Row, assignee: Optional[str]) -> bool:
        if not assignee:
            result.skipped_unassigned.append(row["id"])
            return False
        # Skip tasks whose assignee is not a real Hermes profile. Control-plane
        # lanes are pulled by terminals via claim_task directly and should not
        # be auto-spawned.
        try:
            from hermes_cli.profiles import profile_exists  # local import: avoids cycle
        except Exception:
    ...
    def _note_profile_spawn(assignee: Optional[str]) -> None:
        if _per_profile_cap is not None and assignee:
            _per_profile_running[assignee] = _per_profile_running.get(assignee, 0) + 1

    def _spawn_ready(row: sqlite3.Row) -> bool:
        nonlocal spawned
        if _total_full():
            return False
        row_assignee = _resolve_ready_assignee(row)
        if row_assignee is None:
            return False
        if not _profile_spawnable(row, row_assignee):
            return False
        ...
        if dry_run:
            ...
            spawned += 1
            _note_profile_spawn(row_assignee)
            return True
        ...
        try:
            ...
            spawned += 1
            _note_profile_spawn(claimed.assignee)
            return True
        except Exception as exc:
            auto = _record_spawn_failure(
